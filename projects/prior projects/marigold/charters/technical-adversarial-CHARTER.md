# technical-adversarial — Adversarial Charter
...
1. Read your memory file: `memory/technical-adversarial-memory.md`
...
When reviewing **web-001** (website implementation), assess:

- **Brand fidelity** — Does the site use the exact hex values from the selected brand direction? Are the specified Google Fonts loaded and applied correctly? Does the imagery treatment match the approved direction?
- **Architecture fidelity** — Does the site implement the approved navigation structure, page hierarchy, and section ordering? Are all pages present? Do wireframe descriptions match the built layout?
- **Copy fidelity** — Is all approved copy placed verbatim in the correct sections? Has any copy been rewritten, truncated, or omitted?
- **Responsive design** — Does the site render correctly across desktop (1200px+), tablet (768-1199px), and mobile (below 768px)? Are there layout breaks, overflow issues, or unreadable text at any breakpoint?
- **Accessibility** — Semantic HTML5 structure, proper heading hierarchy (no skipped levels), alt text on all images, sufficient color contrast (WCAG AA minimum), keyboard navigation, ARIA labels on interactive elements.
- **Performance** — No unnecessary dependencies, no render-blocking issues, reasonable file sizes. Vanilla HTML/CSS/JS only — no frameworks.
- **Functionality** — Do all links work? Do booking CTAs point to the placeholder URL? Do email capture forms function? Does mobile navigation work? Do any JS interactions fail?
- **Runnability** — Can the site be opened directly in a browser without a build step or server? Are instructions clear for a non-technical user?
...
## Memory
Your memory file: `memory/technical-adversarial-memory.md`
